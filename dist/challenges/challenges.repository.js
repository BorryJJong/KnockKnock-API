...
let ChallengesRepository = class ChallengesRepository extends typeorm_1.Repository {
    async checkExistChallenge({ id }) {
        ...
        if (challenge) {
            throw new common_1.UnauthorizedException('이미 존재하는 챌린지입니다.');
        }
    }
    async findChallengeById(id) {
        const challenge = await this.findOne({ select: ['id', 'title', 'subTitle', 'content', 'regDate'], where: { id } });
        if (!challenge) {
            throw new common_1.UnauthorizedException('존재하지 않는 챌린지입니다.');
        }
        return challenge;
    }
    ...
    async getChallengeList() {
        const userPostChallenge = (0, typeorm_1.getManager)().createQueryBuilder()
        ...
        const challengePostCnt = (0, typeorm_1.getManager)().createQueryBuilder()
            ...
            .groupBy("a.id");
        const challengeList = (0, typeorm_1.getManager)().createQueryBuilder()
            .select("ma.id", "id")
    ...
    async getParticipantList(challengeId) {
        ...
        const challengePostCnt = (0, typeorm_1.getManager)().createQueryBuilder()
            ...
            .innerJoin("(" + userPostChallenge.getQuery() + ")", "b", "a.id = b.challenge_id")
            .groupBy("a.id");
        const participantList = (0, typeorm_1.getManager)().createQueryBuilder()
            .select("ma.id", "id")
